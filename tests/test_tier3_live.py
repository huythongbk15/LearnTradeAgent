...
from trading_agent.exchanges.models import (
    Symbol, AssetClass, MarketType, OrderSide, OrderType, OrderStatus,
...
class PaperAdapter(ExchangeAdapter):
    ...
    def __init__(self, exchange_id: str, prices: dict[str, float], fee_rate: float = 0.001):
    ...
    async def fetch_ticker(self, symbol: Symbol) -> Ticker:
        ...
        return Ticker(
            symbol=symbol, timestamp=datetime.now(), bid=Decimal(str(p * 0.9998)),
            ask=Decimal(str(p * 1.0002)), last=Decimal(str(p)),
        )
    ...
    async def fetch_balance(self) -> dict[AssetClass, Balance]:
        return {
            AssetClass.CRYPTO: Balance(
                ...
                assets={"USDT": {"free": float(self._buying_power), "used": 0.0,
    ...
    async def fetch_positions(self, symbol: Optional[Symbol] = None) -> list[Position]:
        ...
        for sym_id, qty in self._positions.items():
            ...
            pos.append(Position(
                symbol=s, size=qty, entry_price=Decimal(str(price)),
    ...
    async def fetch_open_orders(self, symbol: Optional[Symbol] = None) -> list[Order]:
        return [o for o in self._orders if o.status in (OrderStatus.OPEN, OrderStatus.PARTIAL)]
...
def test_router_best_price_routes_to_cheapest_venue(router):
    ...
    orders = _run(r.smart_order(sym, OrderSide.BUY, Decimal("0.1"),
...
def test_router_twap_splits_into_slices(router):
    ...
    r, _, _ = router
    sym = crypto_symbol("BTC", "USDT", "binance")
    plan = _run(r.create_execution_plan(
        sym, OrderSide.BUY, Decimal("1.0"), strategy=RoutingStrategy.TWAP,
...
def test_router_execution_quality_tracks_fills(router):
    ...
    r, _, _ = router
    sym = crypto_symbol("BTC", "USDT", "binance")
    _run(r.smart_order(sym, OrderSide.BUY, Decimal("0.5"), strategy=RoutingStrategy.BEST_PRICE))
...
def test_regime_switching_backtest_runs_on_engine():
    ...
    print(f"  Regime backtest: return={result['total_return']:.2%} "
...
def test_paper_order_buy_sell_roundtrip():
    ...
    buy = Order(id="b1", symbol=sym, side=OrderSide.BUY, type=OrderType.MARKET, size=Decimal("1"))
    ...
    sell = Order(id="s1", symbol=sym, side=OrderSide.SELL, type=OrderType.MARKET, size=Decimal("1"))
...
def test_paper_order_rejects_oversell():
    pa = PaperAdapter("paper", {"BTC/USDT": 50000.0})
    sym = crypto_symbol("BTC", "USDT", "paper")
    sell = Order(id="s1", symbol=sym, side=OrderSide.SELL, type=OrderType.MARKET, size=Decimal("5"))
...
def test_livebroker_get_account_alpaca():
    ...
    pa.get_account_info = lambda: {
        "id": "abc123", "status": "ACTIVE", "cash": 9999.0, "equity": 15000.0,
        "buying_power": 20000.0, "initial_margin": 0, "maintenance_margin": 0,
    }
...
def test_livebroker_positions_and_order_roundtrip():
    ...
    buy = Order(id="lb_b1", symbol=sym, side=OrderSide.BUY,
                type=OrderType.MARKET, size=Decimal("1.0"))
...
def test_livebroker_dry_order_build():
    pa = PaperAdapter("alpaca", {"BTC/USDT": 100.0})
    _run(pa.connect())
    order = _run(pa.create_order(Order(
        id="x1", symbol=crypto_symbol("BTC", "USDT", "alpaca"),
        side=OrderSide.BUY, type=OrderType.MARKET, size=Decimal("0.1"),
    )))
