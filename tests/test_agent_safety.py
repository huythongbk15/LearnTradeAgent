...
def test_high_risk_is_a_hard_gate_and_trader_is_reported_once(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        orchestrator,
        "_build_context",
        lambda df, symbol, timeframe, price, current_position, portfolio: (
            AnalysisContext(
                symbol=symbol,
                timeframe=timeframe,
                current_price=price,
