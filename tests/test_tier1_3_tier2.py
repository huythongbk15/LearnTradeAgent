...
class TestSlippageModel:
    ...
    def test_slippage_scales_with_size(self):
        ...
        print(f"  0.1 BTC: {small:.1f} bps → 10 BTC: {large:.1f} bps ({large/small:.1f}x)")

    def test_slippage_book_depth_penalty(self):
        from src.trading_agent.execution.smart_router import SlippageModel
        sm = SlippageModel(k=1.0)
        no_depth = sm.estimate_slippage_bps(qty=10.0, adv=100_000, volatility=0.05)
        shallow = sm.estimate_slippage_bps(qty=10.0, adv=100_000, volatility=0.05, book_depth_usd=10_000)
...
class TestSmartExecution:
    ...
    def test_twap_order_creation(self):
        ...
        order = engine.create_twap_order("BTC/USDT", "buy", 1.0, duration_s=100, n_slices=10)
    ...
    def test_vwap_order_creation(self):
        ...
        order = engine.create_vwap_order("ETH/USDT", "sell", 8.0, profile, duration_s=80)
    ...
    def test_twap_execute_dry_run(self):
        from src.trading_agent.execution.smart_router import SmartExecutionEngine
        engine = SmartExecutionEngine()
        order = engine.create_twap_order("BTC/USDT", "buy", 0.5, duration_s=10, n_slices=5)
        ...
        print(f"  TWAP dry-run: filled {summary['filled_qty']:.4f} @ avg {summary['avg_price']:.0f}")
...
class TestAlphaLibrary:
    ...
    def test_compute_alpha(self, sample_ohlcv):
        ...
        print(f"  RSI_14: {len(valid)} valid, range [{valid.min():.1f}, {valid.max():.1f}]")
...
class TestAlphaEvaluator:
    def test_evaluate_good_alpha(self, sample_ohlcv):
        ...
        alpha = forward_ret + np.random.randn(len(forward_ret)) * 0.001  # nearly perfect
        report = evaluator.evaluate(alpha, forward_ret, name="perfect_alpha")
        assert report.grade in ("A", "B"), f"Near-perfect alpha got grade {report.grade}"
    ...
    def test_evaluate_random_alpha(self, sample_ohlcv):
        ...
        assert abs(report.ic_mean) < 0.15, f"Random alpha IC {report.ic_mean:.4f} too high"
...
class TestAutoMLPipeline:
    def test_scan(self, sample_ohlcv, tmp_path):
        from src.trading_agent.alpha_research.pipeline import _make_library, AlphaEvaluator, AutoMLPipeline
        lib = _make_library()
        evaluator = AlphaEvaluator(forward_periods=5)
        ...
        print(f"  AutoML scan: {report['total_alphas']} alphas, top={report['top_10'][0]['name']}")
