...
class MockTechnicalAnalyst:
    ...
    def analyze(self, context: AnalysisContext) -> AgentMessage:
        ...
        sma_20 = float(sma_20[-1]) if len(sma_20) > 0 else context.current_price
        sma_50 = close.rolling_mean(50).tail(1)
        sma_50 = float(sma_50[-1]) if len(sma_50) > 0 else context.current_price
        ...
        if self.use_llm:
            ...
            signal = (
                "BUY"
                if resp and "buy" in resp.lower()
                else ("SELL" if resp and "sell" in resp.lower() else "HOLD")
            )
...
class MockSentimentAnalyst:
    ...
    def analyze(self, context: AnalysisContext) -> AgentMessage:
        ...
        if self.use_llm:
            prompt = (
                f"Volume ratio {vol_ratio:.2f}. Sentiment: bullish/bearish/neutral?"
            )
            resp = _llm_chat(prompt, max_tokens=20)
            signal = (
                "BUY"
                if resp and "bull" in resp.lower()
                else ("SELL" if resp and "bear" in resp.lower() else "HOLD")
            )
    ...
    def analyze(self, context: AnalysisContext) -> AgentMessage:
        ...
        if not all([tech_msg, sent_msg, risk_msg]):
            return AgentMessage(
                ...
                signal="HOLD",
                confidence=0.5,
                reasoning="Missing agent signals",
            )
        ...
        for role, w in weights.items():
            msg = {
                "technical_analyst": tech_msg,
                "sentiment_analyst": sent_msg,
                "risk_manager": risk_msg,
            }[role]
        ...
        if self.use_llm:
            ...
            signal = (
                "BUY"
                if final_score > 0.2
                else ("SELL" if final_score < -0.2 else "HOLD")
            )
...
def run_comparison(
    days: int = 30, symbol: str = "BTC/USDT", timeframe: str = "1h", max_bars: int = 50
):
    ...
    for label, signals in [("LLM", llm_signals), ("Rule", rule_signals)]:
        ...
        total = len(signals)
        print(
            f"  {label}: BUY={buy} ({buy / total * 100:.1f}%), SELL={sell} ({sell / total * 100:.1f}%), HOLD={hold} ({hold / total * 100:.1f}%)"
        )
    ...
    # Confidence
    print(
        f"Avg confidence - LLM: {sum(llm_confs) / len(llm_confs):.3f}, Rule: {sum(rule_confs) / len(rule_confs):.3f}"
    )
    ...
    for agent_key, agent_label in [
        ...
        agr = sum(1 for left, right in zip(llm_a, rule_a) if left == right)
        print(
            f"  {agent_label} agreement: {agr}/{len(llm_a)} ({agr / len(llm_a) * 100:.1f}%)"
        )

    # Sample output
    print("\nFirst 15 bars:")
    print(
        f"{'Bar':>4} {'Price':>10} {'LLM':>6} {'LLM_C':>6} {'Rule':>6} {'Rule_C':>6} {'Match'}"
    )
    for i in range(min(15, len(results["llm"]))):
        left = results["llm"][i]
        right = results["rule"][i]
        match = "✓" if left["trader"] == right["trader"] else "✗"
        print(
            f"{i:>4} {left['price']:>10.0f} {left['trader']:>6} {left['trader_conf']:>6.2f} {right['trader']:>6} {right['trader_conf']:>6.2f}   {match}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-bars", type=int, default=50, help="Max bars to test (default 50)"
    )
