...
def add_regime_indicators(
    df: pl.DataFrame, atr_period: int = 14, lookback: int = 252
) -> pl.DataFrame:
    """
    ...
    plus_di = 100 * (
        plus_dm.rolling_mean(window_size=atr_period) / (pl.col("atr") + 1e-9)
    )
    minus_di = 100 * (
        minus_dm.rolling_mean(window_size=atr_period) / (pl.col("atr") + 1e-9)
    )
    ...
    vol_regime = (
        pl.when(pl.col("atr_pctl") < 0.33)
        .then(pl.lit("low_vol"))
        .when(pl.col("atr_pctl") < 0.67)
        .then(pl.lit("mid_vol"))
