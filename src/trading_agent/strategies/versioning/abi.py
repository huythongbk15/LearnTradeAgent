    ...
    def from_strategy(cls, strategy_class: type) -> "StrategyABI":
        ...
        for name, method in inspect.getmembers(strategy_class, predicate=inspect.isfunction):
            ...
            for pname, p in method_sig.parameters.items():
                ...
                method_params.append(ParameterSpec(
                    name=pname,
    ...
    def _detect_required_data(strategy_class: type) -> list[str]:
        ...
        for name, method in inspect.getmembers(strategy_class, predicate=inspect.isfunction):
            ...
            for pname, p in sig.parameters.items():
                ...
                if any(kw in type_str for kw in ["candle", "ohlcv", "tick", "orderbook", "trade"]):
        ...
        for name in old_params:
            if name not in new_params:
                ...
                if old_p.type != new_p.type:
                    ...
                    result["warnings"].append(f"Parameter {name} type changed: {old_p.type} -> {new_p.type}")
                ...
                if not old_p.required and new_p.required:
                    result["breaking_changes"].append(f"Parameter {name} became required")
                    result["compatible"] = False
        
        for name in new_params:
            if name not in old_params:
                if new_params[name].required:
                    result["breaking_changes"].append(f"Added required parameter: {name}")
                    result["compatible"] = False
                else:
