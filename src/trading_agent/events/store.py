...
class EventStoreBackend(ABC):
    ...
    @abstractmethod
    async def subscribe(self, stream_name: str, handler, consumer_group: str = "") -> str:
        pass
...
class RedisEventStore(EventStoreBackend):
    ...
    async def append_batch(self, events: list[Event]) -> list[str]:
        ...
        for event in events:
            stream = self._stream_name(event.event_type)
            pipe.xadd(stream, {"data": json.dumps(event.to_dict())}, 
                     maxlen=self.config.max_stream_length, approximate=True)
    ...
    async def read_all(self, start: str | int = "0", count: int = 100) -> list[Event]:
        ...
        for stream in streams:
            entries = await self._client.xrange(stream, min=start, max="+", count=count)
            all_events.extend([Event.from_dict(json.loads(e[1]["data"])) for e in entries])
...
class EventStore:
    ...
    async def read_stream(self, stream_name: str, start: str | int = "0", count: int = 100) -> list[Event]:
        if not self._backend:
            raise RuntimeError("Not connected")
    ...
    def create_trade_event(
        ...
        fee: Decimal, fee_currency: str, exchange: str, order_id: str,
        strategy_id: str, is_maker: bool = False
    ) -> TradeEvent:
        return TradeEvent(
            symbol=symbol, side=side, size=size, price=price,
            fee=fee, fee_currency=fee_currency, exchange=exchange,
            order_id=order_id, strategy_id=strategy_id, is_maker=is_maker
        )
    
    def create_signal_event(
        self, symbol: str, signal_type: str, strength: float,
        strategy_id: str, timeframe: str, indicators: dict = None,
        regime: str = ""
    ) -> SignalEvent:
        return SignalEvent(
            symbol=symbol, signal_type=signal_type, strength=strength,
            strategy_id=strategy_id, timeframe=timeframe,
            indicators=indicators or {}, regime=regime
        )
    
    def create_risk_event(
        self, check_type: str, passed: bool, metric: str,
        value: Decimal, threshold: Decimal, symbol: str = "",
        strategy_id: str = "", details: str = ""
    ) -> RiskEvent:
        return RiskEvent(
            check_type=check_type, passed=passed, metric=metric,
            value=value, threshold=threshold, symbol=symbol,
            strategy_id=strategy_id, details=details
        )
    
    def create_order_event(
        self, order_id: str, symbol: str, side: str, order_type: str,
        size: Decimal, price: Decimal | None, status: str,
        filled_size: Decimal = Decimal(0), avg_fill_price: Decimal = Decimal(0),
        exchange: str = "", strategy_id: str = "", error: str = ""
    ) -> OrderEvent:
        return OrderEvent(
            order_id=order_id, symbol=symbol, side=side, order_type=order_type,
            size=size, price=price, status=status, filled_size=filled_size,
            avg_fill_price=avg_fill_price, exchange=exchange,
            strategy_id=strategy_id, error=error
        )
    
    def create_position_event(
        self, symbol: str, size: Decimal, entry_price: Decimal,
        mark_price: Decimal, unrealized_pnl: Decimal, realized_pnl: Decimal,
        leverage: Decimal, strategy_id: str
    ) -> PositionEvent:
        return PositionEvent(
            symbol=symbol, size=size, entry_price=entry_price,
            mark_price=mark_price, unrealized_pnl=unrealized_pnl,
            realized_pnl=realized_pnl, leverage=leverage,
            strategy_id=strategy_id
        )
    
    def create_portfolio_event(
        self, portfolio_id: str, total_value: Decimal, cash: Decimal,
        positions_value: Decimal, drawdown_pct: Decimal,
        strategy_weights: dict
    ) -> PortfolioEvent:
        return PortfolioEvent(
            portfolio_id=portfolio_id, total_value=total_value,
            cash=cash, positions_value=positions_value,
            drawdown_pct=drawdown_pct, strategy_weights=strategy_weights
        )