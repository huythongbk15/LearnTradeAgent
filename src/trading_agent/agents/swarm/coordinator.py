...
class CoordinatorAgent(Agent):
            ...
            async with semaphore:
                try:
                    ...
                    return name, await asyncio.wait_for(
                        agent.process(agent_data), timeout=self.config.timeout_seconds
                    )
    ...
    def _aggregate(
        ...
        if self.config.mode == SwarmMode.CONSENSUS:
            final = self._consensus_aggregate(
                trading_signals, risk_approved, risk_warnings
            )
        elif self.config.mode == SwarmMode.PIPELINE:
            final = self._pipeline_aggregate(
                trading_signals, risk_approved, risk_warnings
            )
        else:  # HIERARCHICAL
            final = self._hierarchical_aggregate(
                trading_signals, risk_approved, risk_warnings
            )
...
def weighted_average(signals: list[AgentSignal], weights: dict[str, float]) -> dict:
    ...
    return {
        ...
        "size_pct": size_weights[best_action] / action_weights[best_action]
        if action_weights[best_action] > 0
        else 0,
    }
