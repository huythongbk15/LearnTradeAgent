...
class RiskController:
    ...
    def check_all(self) -> list[str]:
        ...
        if (
            ...
            and datetime.now(UTC) >= self._cooldown_until
        ):
        ...
        if self._circuit_breaker_active:
            self._alerts.append(
                f"⚠️  CIRCUIT BREAKER ACTIVE: {self._circuit_breaker_reason}"
            )
            return self._alerts
        ...
        if self._cooldown_until and datetime.now(UTC) < self._cooldown_until:
            remaining = (
                self._cooldown_until - datetime.now(UTC)
            ).total_seconds() / 3600
    ...
    def _check_max_drawdown(self, equity: float):
        ...
        if drawdown >= self.max_drawdown_pct:
            msg = (
                f"🔴 MAX DRAWDOWN TRIGGERED: {drawdown * 100:.1f}% "
    ...
    def _check_position_concentration(self):
        ...
        for pos in self.engine.exchange.get_all_positions():
            ...
            if pos_pct > self.max_position_pct:
                msg = (
                    f"⚠️ Position concentration: {pos.symbol} is {pos_pct * 100:.0f}% "
    ...
    def record_trade(
        self, pnl: float, entry_price: float, exit_price: float, size: float
    ):
    ...
    def calculate_position_size(
        ...
        vol_scale = (
            min(target_daily_vol / est_daily_vol, 2.0) if est_daily_vol > 0 else 1.0
        )
    ...
    def get_dynamic_stop_loss(
        self, entry_price: float, atr: float, regime_info: Optional[dict] = None
    ) -> float:
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cooldown_active": bool(
                self._cooldown_until and datetime.now(UTC) < self._cooldown_until
            ),
    ...
    def update_atr_trailing_stops(self, ohlcv_data: dict[str, Any]):
        ...
        for pos in self.engine.exchange.get_all_positions():
            ...
            if latest_stop and latest_stop > 0:
                if side == "long" and (
                    pos.stop_loss is None or latest_stop > pos.stop_loss
                ):
                    pos.stop_loss = latest_stop
                    pos.metadata["trailing_stop_type"] = "atr"
                    pos.metadata["trailing_atr_mult"] = atr_mult
                    logger.debug(
                        f"ATR trailing stop updated: {pos.symbol} @ {latest_stop:.2f} (mult={atr_mult})"
                    )
                elif side == "short" and (
                    pos.stop_loss is None or latest_stop < pos.stop_loss
                ):
