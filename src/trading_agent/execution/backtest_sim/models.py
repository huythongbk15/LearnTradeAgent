...
class ExecutionSimulator:
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        if order_id in self.state.open_orders:
            ...
            # Remove from queue
            self.state.order_queue = [o for o in self.state.order_queue if o.order_id != order_id]
            return True
        return False
    ...
    def _fill_limit_as_taker(self, order: SimulatedOrder, book: OrderBookSnapshot) -> SimulatedFill:
        ...
        del self.state.open_orders[order.order_id]
        self.state.order_queue = [o for o in self.state.order_queue if o.order_id != order.order_id]
        
        return fill
    
    def _fill_limit_as_maker(self, order: SimulatedOrder, book: OrderBookSnapshot) -> SimulatedFill:
        ...
        impact = self._compute_impact(book, order.quantity, order.side) * 0.5  # Reduced impact
        if order.side == OrderSide.BUY:
    ...
    def _get_queue_position(self, order: SimulatedOrder) -> int:
        ...
        for o in self.state.order_queue:
            if o.symbol == order.symbol and o.side == order.side and o.price == order.price:
    ...
    def _sample_latency(self) -> float:
        ...
        if self.config.latency_distribution == "lognormal":
            ...
            mu = np.log(self.config.base_latency_ms) - 0.5 * (self.config.latency_jitter_ms / self.config.base_latency_ms) ** 2
            ...
            return float(max(1.0, self.state.rng.normal(self.config.base_latency_ms, self.config.latency_jitter_ms)))
        else:  # uniform
            return float(self.state.rng.uniform(
                self.config.base_latency_ms - self.config.latency_jitter_ms,
                self.config.base_latency_ms + self.config.latency_jitter_ms
            ))
    
    def _compute_slippage(self, book: OrderBookSnapshot, quantity: float, side: OrderSide) -> float:
        ...
        if book_depth > 0:
            size_factor = 1.0 + (quantity / book_depth) * 0.5  # 0.5x slippage at 10% of depth
        else:
            size_factor = 1.5
    ...
    def _compute_impact(self, book: OrderBookSnapshot, quantity: float, side: OrderSide) -> float:
    ...
    def get_total_filled(self, order_id: str) -> float:
        ...
        return sum(f.quantity for f in self.state.fill_history if f.order_id == order_id)
