...
def calculate_risk_parity_weights(
    ...
    for _ in range(1000):
        ...
        mrc = [
            sum(weights[j] * cov_matrix[i][j] for j in range(n)) / port_var
            for i in range(n)
        ]
    ...
    def calculate_position_size(
        ...
        if method == "fixed":
            ...
            risk_amount = equity * min(
                self.params.target_annual_vol / current_vol, self.params.max_leverage
            )
            ...
            if len(self.trade_history) >= 30:
                ...
                risk_amount = (
                    equity * optimal_f * self.params.kelly_fraction
                )  # Scale down
            else:
...
def vol_target_size(
    equity: float, current_vol: float, target_vol: float = 0.15, max_lev: float = 2.0
) -> float:
...
if __name__ == "__main__":
    ...
    print(
        f"Optimal f: {calculate_optimal_f([t['pnl'] for t in sizer.trade_history]):.4f}"
    )
    print(
        f"Position size (100k equity, 50k price): {sizer.calculate_position_size(100000, 50000):.6f}"
    )
