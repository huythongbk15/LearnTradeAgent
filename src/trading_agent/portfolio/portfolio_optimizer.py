...
class PortfolioOptimizer:
    ...
    def _make_positive_definite(self, matrix: np.ndarray, epsilon: float = 1e-8) -> np.ndarray:
    ...
    def _equal_weight(self) -> OptimizationResult:
        ...
        return OptimizationResult(
            ...
            sharpe_ratio=Decimal(str((port_return - self.risk_free_rate) / port_vol)) if port_vol > 0 else Decimal('0'),
            method=self.method,
        )
    ...
    def _min_variance(self) -> OptimizationResult:
        ...
        if not result.success:
            return OptimizationResult(weights={}, expected_return=Decimal('0'), expected_volatility=Decimal('0'), sharpe_ratio=Decimal('0'), method=self.method, success=False, message=result.message)

        weights = {self._symbols[i]: Decimal(str(max(0, w))) for i, w in enumerate(result.x)}
    ...
    def _max_sharpe(self) -> OptimizationResult:
        ...
        if self.constraints.target_return is not None:
            constraints.append({'type': 'eq', 'fun': lambda w: w @ mu - self.constraints.target_return})
    ...
    def _mean_variance_constrained(self) -> OptimizationResult:
        ...
        if self.constraints.max_turnover is not None and self._current_weights:
            ...
            constraints.append({'type': 'ineq', 'fun': lambda w: self.constraints.max_turnover - np.sum(np.abs(w - curr))})
    ...
    def _hrp(self) -> OptimizationResult:
        ...
        def get_rec_bipart(cov, items):
            ...
            alpha = 1 - left_vol / (left_vol + right_vol) if (left_vol + right_vol) > 0 else 0.5
        ...
        weights = {sorted_symbols[i]: Decimal(str(weights_dict.get(i, 0))) for i in range(len(sorted_symbols))}
        return self._create_result(weights)
    ...
    def _black_litterman(
        ...
        delta = (market_portfolio_return - self.risk_free_rate) / (market_portfolio_vol ** 2)
        pi = delta * cov @ w_mkt
        ...
        P_cov_P = P @ cov @ P.T
        omega = np.diag(np.diag(P_cov_P) * (1 - np.array(confidence_list)) / np.array(confidence_list))
        ...
        return OptimizationResult(
            ...
            sharpe_ratio=Decimal(str((port_return - self.risk_free_rate) / port_vol)) if port_vol > 0 else Decimal('0'),
            method=self.method,
            posterior_returns={s: Decimal(str(posterior_mu[i])) for i, s in enumerate(self._symbols)},
    ...
    def _resampled_ef(self, n_portfolios: int = 500) -> OptimizationResult:
        ...
        for _ in range(n_portfolios):
            sample_idx = np.random.choice(len(self._returns), len(self._returns), replace=True)
            ...
            res = minimize(obj, x0, method='SLSQP', bounds=bounds, constraints=constraints, options={'maxiter': 500})
            if res.success:
                w = res.x
        ...
        weights = {self._symbols[i]: Decimal(str(max(0, w))) for i, w in enumerate(avg_weights)}
        total = sum(weights.values())
        if total > 0:
            weights = {k: v / total for k, v in weights.items()}
    ...
    def _risk_parity(self) -> OptimizationResult:
        ...
        if not rb_result.success:
            return OptimizationResult(weights={}, expected_return=Decimal('0'), expected_volatility=Decimal('0'), sharpe_ratio=Decimal('0'), method=self.method, success=False, message=rb_result.message or "Risk parity failed")
    ...
    def efficient_frontier(
        ...
        for target in target_returns:
            ...
            res = minimize(obj, x0, method='SLSQP', bounds=bounds, constraints=constraints, options={'maxiter': 1000})
            if res.success:
                w = res.x
                w = np.maximum(w, 0)
                w = w / w.sum() if w.sum() > 0 else w
                frontier_weights.append({self._symbols[i]: Decimal(str(w[i])) for i in range(n)})
    ...
    def monte_carlo_simulation(
        ...
        for _ in range(n_simulations):
            random_returns = np.random.multivariate_normal(mu / 252, cov / 252, time_horizon)
        ...
        return {
            ...
            'prob_10pct_gain': float(np.mean(np.array(final_values) > initial_value * 1.1)),
            'prob_20pct_gain': float(np.mean(np.array(final_values) > initial_value * 1.2)),
