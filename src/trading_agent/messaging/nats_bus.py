...
class NATSBus(MessageBus):
    ...
    def _stream_name(
        self, topic: str, priority: MessagePriority = MessagePriority.NORMAL
    ) -> str:
    ...
    async def _consume(
        self, subscription_id: str, psub: Any, handler: MessageHandler
    ) -> None:
    ...
    async def request(
        ...
        # Subscribe to reply
        reply_sub_id = await self.subscribe(
            reply_subject, reply_handler, durable=f"reply-{correlation_id}"
        )

        try:
            # Send request
            await self.publish(
                topic, payload, correlation_id=correlation_id, reply_to=reply_subject
            )
