...
class RegionSyncController:
    ...
    def __init__(
        ...
        self.primary_region = next(
            (r for r in regions if r.role == RegionRole.PRIMARY), None
        )
        ...
        while self._running:
            try:
                ...
                region.status = (
                    ...
                    else RegionStatus.UNHEALTHY
                )

                if region.error_count >= 3:
                    logger.warning(
                        f"Region {region.name} marked unhealthy, initiating failover check"
                    )
    ...
    async def _sync_region(self, region: RegionInfo):
        ...
        if region.last_sync:
            region.sync_lag_seconds = (
                datetime.utcnow() - region.last_sync
            ).total_seconds()
    ...
    async def _sync_configmaps(self, region: RegionInfo):
        ...
        primary_cms = self.k8s_core.list_namespaced_config_map(
            namespace="trading-agent", label_selector="app=trading-agent"
        )
        ...
        for secret_name in safe_secrets:
            try:
                secret = self.k8s_core.read_namespaced_secret(
                    name=secret_name, namespace="trading-agent"
                )
    ...
    async def _check_region_health(self, region: RegionInfo):
        ...
        try:
            ...
            if region.role != RegionRole.PRIMARY and region.last_sync:
                ...
                if lag > self.sync_policy.max_lag_seconds:
                    logger.warning(
                        f"Region {region.name} sync lag: {lag:.0f}s (max: {self.sync_policy.max_lag_seconds}s)"
                    )
                    region.status = RegionStatus.DEGRADED
    ...
    async def _promote_secondary(self):
        ...
        secondary = next(
            (r for r in self.regions.values() if r.role == RegionRole.SECONDARY), None
        )
    ...
    async def _promote_tertiary(self):
        ...
        tertiary = next(
            (r for r in self.regions.values() if r.role == RegionRole.TERTIARY), None
        )
...
async def manual_failover(target_region: str):
    ...
    if controller.primary_region:
        ...
        await controller._update_region_config(
            controller.primary_region, is_primary=False
        )
