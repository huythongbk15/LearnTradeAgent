...
class ChaosExperimentRunner(ABC):
    ...
    async def run(self, metrics_collector=None) -> ExperimentResult:
        ...
        duration = (
            self.experiment.end_time - self.experiment.start_time
        ).total_seconds()
...
class PodKillExperiment(ChaosExperimentRunner):
    ...
    async def inject_fault(self) -> bool:
        label_selector = ",".join(
            f"{k}={v}" for k, v in self.experiment.target_labels.items()
        )
    ...
    async def recover(self) -> bool:
        ...
        for _ in range(30):  # Wait up to 5 minutes
            ...
            if ready and len(pods.items) >= self.experiment.parameters.get(
                "min_replicas", 1
            ):
                return True
    ...
    async def verify_impact(self) -> dict[str, Any]:
        ...
        if len(pods.items) < self.experiment.parameters.get("min_replicas", 1):
            ...
            recommendations.append(
                "Increase replica count or add pod disruption budget"
            )
...
class CPUStressExperiment(ChaosExperimentRunner):
    ...
    async def verify_impact(self) -> dict[str, Any]:
        return {
            "observations": [
                f"CPU stressed to {self.experiment.parameters.get('cpu_percent', 80)}%"
            ],
            "recommendations": [
...
class ChaosExperimentSuite:
    ...
    def add_pod_kill(
        self, name: str, target_labels: dict, duration: int = 60, kill_count: int = 1
    ):
        exp = ChaosExperiment(
            name=name,
    ...
    def add_network_latency(
        self, name: str, target_labels: dict, duration: int = 60, latency_ms: int = 100
    ):
        exp = ChaosExperiment(
            name=name,
    ...
    def add_exchange_api_failure(
        self, name: str, target_labels: dict, duration: int = 60
    ):
        exp = ChaosExperiment(
            name=name,
    ...
    def add_cpu_stress(
        self, name: str, target_labels: dict, duration: int = 60, cpu_percent: int = 80
    ):
        exp = ChaosExperiment(
            name=name,
        ...
        for exp in self.experiments:
            if self.dry_run:
                ...
                if runner:
                    ...
                    logger.warning(
                        f"No runner for experiment type: {exp.experiment_type}"
                    )
                    continue
    ...
    async def _dry_run_experiment(self, exp: ChaosExperiment) -> ExperimentResult:
        ...
        logger.info(
            f"[dry-run] Simulated {exp.experiment_type.value} on {exp.target_labels}"
        )
        return ExperimentResult(
