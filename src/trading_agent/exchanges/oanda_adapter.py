...
if not OANDA_AVAILABLE:
    logger.warning(
        "oandapyV20 not installed — OANDAAdapter disabled (pip install oandapyV20)"
    )
...
class OANDAAdapter:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"OANDA connected: account={self.config.account_id}, env={self.config.environment}"
            )
    ...
    async def fetch_balance(self) -> dict[AssetClass, Balance]:
        ...
        try:
            ...
            return {
                AssetClass.FOREX: Balance(asset_class=AssetClass.FOREX, assets=assets)
            }
        except V20Error as e:
    ...
    async def fetch_positions(self, symbol: Symbol | None = None) -> list[Position]:
        ...
        try:
            ...
            for pos in r.response.get("positions", []):
                ...
                long_units = (
                    Decimal(str(pos["long"]["units"]))
                    if pos["long"]["units"] != "0"
                    else Decimal(0)
                )
                short_units = (
                    Decimal(str(pos["short"]["units"]))
                ...
                result.append(
                    Position(
                        ...
                        mark_price=Decimal(
                            str(
                                pos["long"]["currentPrice"]
                                if net_units > 0
                                else pos["short"]["currentPrice"]
                            )
                        ),
    ...
    async def fetch_ticker(self, symbol: Symbol) -> Ticker:
        ...
        try:
            ...
            r = pricing.PricingInfo(
                accountID=self.config.account_id, params={"instruments": inst}
            )
            self._client.request(r)
            ...
            return Ticker(
                ...
                last=Decimal(
                    str(
                        (
                            float(price["bids"][0]["price"])
                            + float(price["asks"][0]["price"])
                        )
                        / 2
                    )
                )
    ...
    async def fetch_order_book(self, symbol: Symbol) -> OrderBook:
        ...
        return OrderBook(
            symbol=symbol, timestamp=ticker.timestamp, bids=bids, asks=asks
        )
    ...
    async def fetch_open_orders(self, symbol: Symbol | None = None) -> list[Order]:
        ...
        try:
            r = orders.OrdersList(
                accountID=self.config.account_id, params={"state": "PENDING"}
            )
            self._client.request(r)
            result = []
    ...
    def _build_order_request(self, order: Order, instrument: str) -> dict:
        ...
        units = (
            str(float(order.size))
            if order.side == OrderSide.BUY
            else str(-float(order.size))
        )
    ...
    def _parse_order(self, order_data: dict, symbol: Symbol) -> Order:
        ...
        return Order(
            ...
            time_in_force=self._map_time_in_force_enum(
                order_data.get("timeInForce", "GTC")
            ),
            created_at=datetime.fromisoformat(
                order_data["createTime"].replace("Z", "+00:00")
            ),
        )
