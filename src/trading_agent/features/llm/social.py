...
For each post, output JSON with:
{
  ...
  "time_horizon": "intraday|swing|long_term|unknown"
    ...
    def _aggregate(
        ...
        if n == 0:
            return SocialFeatures(
                symbol=symbol, timestamp=datetime.utcnow(), window_minutes=60
            )
        ...
        for a, p in zip(analyses, posts):
            ...
            influence = (
                p.followers * 0.001 + engagement * 0.1 + (10 if p.verified else 1)
            )
        ...
        avg_topic_sentiment = {t: sum(s) / len(s) for t, s in topic_sentiments.items()}
        ...
        sorted_posts = sorted(
            zip(posts, analyses), key=lambda x: x[0].followers, reverse=True
        )
        top_n = max(1, n // 10)
        ...
        return SocialFeatures(
            ...
            influence_weighted_sentiment=sum(weighted_sentiments)
            / max(1, sum(1 for _ in weighted_sentiments)),
            top_influencers_sentiment=sum(influencer_sentiments)
    ...
    def _detect_coordination(
        self, posts: list[SocialPost], analyses: list[dict]
    ) -> float:
    ...
    async def extract_multi_platform(
        self, platform_posts: dict[str, list[SocialPost]], symbol: str
    ) -> dict[str, SocialFeatures]:
        """Extract features per platform."""
        tasks = [self.extract(posts, symbol) for posts in platform_posts.values()]
    ...
    def get_aggregated(self, symbol: str) -> SocialFeatures | None:
        ...
        return SocialFeatures(
            ...
            influence_weighted_sentiment=sum(
                f.influence_weighted_sentiment for f in feats
            )
            / n,
            top_influencers_sentiment=sum(f.top_influencers_sentiment for f in feats)
            / n,
