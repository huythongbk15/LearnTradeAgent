...
class LLMFeatureSet:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "earnings_revenue_surprise": self.earnings.revenue_surprise
            if self.earnings
            else 0,
            "earnings_guidance": "raised"
            if self.earnings and self.earnings.guidance_raised
            else "lowered"
            if self.earnings and self.earnings.guidance_lowered
            else "inline",
...
class LLMFeaturePipeline:
    ...
    async def extract_all(
        ...
        # Combine
        combined = self._combine_features(symbol, news_feat, earnings_feat, social_feat)
    ...
    def _combine_features(
        ...
        if earnings:
            ...
            earn_sent = np.clip(
                earnings.eps_surprise * 2 + earnings.revenue_surprise, -1, 1
            )
            # Guidance adjustment
        ...
        social_weight = (
            self.social_weight * social_conf * min(1, social.post_count / 100)
        )
        ...
        if social:
            ...
            names.extend(
                [
                    ...
                    "social_bullish",
                    "social_bearish",
                    "social_infl_sentiment",
                    "social_top_infl_sentiment",
                    "social_log_posts",
                    "social_log_authors",
                    "social_log_engagement",
                    "social_sent_momentum",
                    "social_vol_momentum",
                    "social_bot_score",
                    "social_spam_score",
                    "social_coord_score",
    ...
    def get_feature_matrix(
        self, feature_sets: list[LLMFeatureSet]
...
async def demo():
    ...
    print(
        f"Signal: {features.signal_direction} (strength: {features.signal_strength:.3f})"
    )
