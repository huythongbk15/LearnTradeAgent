...
def train(
    data_dir: str = typer.Argument(
        ..., help="Directory with regime data (CSV/Parquet)"
    ),
    algorithm: str = typer.Option(
        "maml", help="Algorithm: maml, reptile, metasgd, anil"
    ),
    ...
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Output file for meta-params"
    ),
):
    ...
    for file in data_path.glob("*.csv"):
        ...
        try:
            ...
            if "close" in df.columns:
                ...
                console.print(
                    f"  Loaded {regime_name}: {len(market_data[regime_name])} points"
                )
    ...
    # Meta-train
    console.print(
        f"[bold]Meta-training with {algorithm.upper()} for {steps} steps...[/bold]"
    )
...
def adapt(
    ...
    regime_data: Path = typer.Argument(
        ..., help="Path to new regime data (CSV/Parquet)"
    ),
    n_samples: int = typer.Option(20, help="Number of samples for adaptation"),
    output: Optional[Path] = typer.Option(
        None, "--output", "-o", help="Output file for adapted params"
    ),
):
    ...
    # Adapt to new regime
    console.print(
        f"[bold]Adapting to new regime ({len(regime_prices)} data points)...[/bold]"
    )
...
def backtest(
    ...
    for i in range(max(ema_slow, bb_period), len(close)):
        ...
        bb_pos = (
            (close[i] - bb_low[i]) / (bb_up[i] - bb_low[i])
            if bb_up[i] != bb_low[i]
            else 0.5
        )
        ...
        if signal != 0 and signal != position:
            ...
            if signal != 0:
                ...
                entry_price = (
                    close[i] * (1 + commission + slippage)
                    if signal > 0
                    else close[i] * (1 - commission - slippage)
                )
...
def compare(
    ...
    console.print(
        "[yellow]Comparison feature - run backtest for each and compare manually[/yellow]"
    )
...
def regimes(
    ...
    for file in sorted(data_path.glob("*.csv")) + sorted(data_path.glob("*.parquet")):
        try:
            ...
            date_start = (
                str(df["timestamp"].iloc[0]) if "timestamp" in df.columns else "N/A"
            )
            date_end = (
                str(df["timestamp"].iloc[-1]) if "timestamp" in df.columns else "N/A"
            )
