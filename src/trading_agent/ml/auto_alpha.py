...
class FeatureImportance:
    ...
    def compute_importance(self, alpha_values: dict[str, np.ndarray],
                           target: np.ndarray | None = None,
                           n_repeats: int = 10) -> list[ImportanceResult]:
        ...
        for name, values in alpha_values.items():
            ...
            results.append(ImportanceResult(
                name=name, importance=max(importance, 0),
                ic_with_target=ic,
            ))
    ...
    def detect_redundancy(self, alpha_values: dict[str, np.ndarray],
...
class AlphaGenerator:
    ...
    UNARY_OPS = ["ema_5", "ema_10", "ema_20", "sma_5", "sma_10", "sma_20",
                 "ts_rank_10", "ts_rank_20", "ts_std_10", "ts_std_20",
                 "log", "abs", "sign", "neg", "delta_1", "delta_5"]
    ...
    def random_tree(self, depth: int = 0) -> AlphaExpression:
        ...
        return AlphaExpression(op=random.choice(self.BINARY_OPS), children=[left, right])

    def evaluate(self, tree: AlphaExpression, data: dict[str, np.ndarray]) -> np.ndarray:
    ...
    def mutate(self, tree: AlphaExpression) -> AlphaExpression:
        ...
        if nodes:
            ...
            target.children = [] if target.op.startswith("data:") else [self.random_tree(1)]
        return mutant
...
class AutoAlphaGenerator:
    ...
    def evolve(self, data: dict[str, np.ndarray], target: np.ndarray,
               population_size: int = 30, n_generations: int = 10,
               tournament_size: int = 5) -> list[dict]:
        """
        ...
        for gen in range(n_generations):
            ...
            for tree in population:
                try:
                    ...
                    scored.append({
                        "tree": tree, "fitness": fitness, "ic": ic,
            ...
            new_pop = [copy.deepcopy(scored[i]["tree"]) for i in range(min(5, len(scored)))]  # elitism
...
if __name__ == "__main__":
    ...
    for r in rankings[:10]:
        print(f"  #{r.rank:2d} {r.name:30s} imp={r.importance:.3f} IC={r.ic_with_target:+.4f}")
