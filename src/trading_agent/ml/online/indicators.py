...
class OnlineMACD(OnlineIndicator):
    ...
    @property
    def is_ready(self) -> bool:
        return self.fast_ema.is_ready and self.slow_ema.is_ready and self.signal_ema.is_ready
...
class OnlineStandardDeviation(OnlineIndicator):
    ...
    @property
    def is_ready(self) -> bool:
        return self.count >= 2 if self.period is None else len(self.values) >= self.period
