...
def bs_price(
    ...
    d1 = (math.log(spot / strike) + (rate + 0.5 * vol**2) * tte) / (
        vol * math.sqrt(tte)
    )
...
class OptionChainProvider:
    ...
    def get_chain(
        self, underlying: str, expiry: str = "", spot: float = 0
    ) -> OptionChain:
    ...
    def get_vol_surface(
        self, underlying: str, spot: float = 0
        ...
        for c in chain1.calls + chain1.puts:
            if c.iv > 0:
                surface.append(
                    VolSurfacePoint(
                        strike=c.strike,
                        expiry=chain1.expiry,
                        tte=self._tte(chain1.expiry),
                        iv=c.iv,
        ...
        for c in chain2.calls + chain2.puts:
            if c.iv > 0:
                surface.append(
                    VolSurfacePoint(
                        strike=c.strike,
                        expiry=chain2.expiry,
                        tte=self._tte(chain2.expiry),
                        iv=c.iv,
                        option_type=c.option_type,
                    )
                )
    ...
    def _compute_skew(self, chain: OptionChain) -> float:
        ...
        calls_25d = [
            c for c in chain.calls if 0.2 < abs(c.greeks.get("delta", 0)) < 0.3
        ]
...
if __name__ == "__main__":
    ...
    for c in sorted(chain.calls, key=lambda x: x.volume, reverse=True)[:5]:
        print(
            f"  K=${c.strike:>10,.0f}  IV={c.iv:.1%}  Vol={c.volume:>6}  Bid/Ask={c.bid:.2f}/{c.ask:.2f}"
        )
    print("\nTop 5 Puts:")
    for p in sorted(chain.puts, key=lambda x: x.volume, reverse=True)[:5]:
        print(
            f"  K=${p.strike:>10,.0f}  IV={p.iv:.1%}  Vol={p.volume:>6}  Bid/Ask={p.bid:.2f}/{p.ask:.2f}"
        )
    print("\nGreeks (ATM Call):")
