...
def run_backtest(df: pl.DataFrame, strategy_name: str, params: dict, capital: float = 10000) -> dict:
...
def walk_forward_optimize(
    ...
    while True:
        ...
        print(f"    Fold {fold}: IS Sharpe={best_sharpe:.3f} | OOS Sharpe={oos_metrics['sharpe']:.3f} | Return={oos_metrics['total_return_pct']:.2f}% | DD={oos_metrics['max_drawdown_pct']:.2f}% | Trades={oos_metrics['num_trades']}")
...
def main():
    ...
    for strategy_name in ["ma_crossover", "rsi", "bbands"]:
        ...
        if results:
            ...
            print(f"    OOS: Return={best_fold['oos_metrics']['total_return_pct']:.2f}% Sharpe={best_fold['oos_metrics']['sharpe']:.3f} DD={best_fold['oos_metrics']['max_drawdown_pct']:.2f}%")
    ...
    for strat, results in all_results.items():
        if results:
            ...
            m = best["oos_metrics"]
            print(f"  {strat:15s} | Sharpe={m['sharpe']:6.3f} | Return={m['total_return_pct']:7.2f}% | DD={m['max_drawdown_pct']:6.2f}% | Trades={m['num_trades']:3.0f} | Params={best['best_params']}")
