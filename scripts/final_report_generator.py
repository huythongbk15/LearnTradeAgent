...
def generate() -> str:
    ...
    lines.append(
        f"**Git SHA:** {__import__('subprocess').check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT).decode().strip()}"
    )
    lines.append("\n---\n")
    ...
    for r in wfo:
        reason = (
            "NO_EDGE"
            if r.get("oos_sharpe", 0) <= 0
            else "COST_SENSITIVE"
            if r.get("cost_2x_sharpe", 0) < 0
            else "PAPER_ELIGIBLE"
        )
        lines.append(
            f"| {r.get('symbol', '')} | {r.get('timeframe', '')} | {r.get('strategy', '')} | {r.get('status', '')} | {reason} |"
        )
    lines.append("")
