#!/usr/bin/env python3
"""Fast multi-pair 1h backtest — last ~1 year (10000 bars)."""
...
BARS = int(os.getenv("BARS", "10000"))  # ~1 year
OUT_DIR = ROOT / "data" / "benchmarks" / "multi_pair_1h_fast"
...
def run_backtest(symbol: str) -> dict:
    ...
    print(f"\n{'='*60}")
    print(f"🚀 Running {symbol} {TIMEFRAME} (last {BARS} bars)")
    print(f"{'='*60}")
    ...
    metrics = {"symbol": symbol, "timeframe": TIMEFRAME, "bars": BARS}
    for line in stdout.splitlines():
        if "Final Equity:" in line:
            try:
                metrics["final_equity"] = float(line.split("Final Equity:")[1].strip().replace("$","").replace(",",""))
            except Exception:
                pass
        if "Total Return:" in line:
            try:
                metrics["total_return_pct"] = float(line.split("Total Return:")[1].strip().replace("%",""))
            except Exception:
                pass
        if "Max Drawdown:" in line:
            try:
                metrics["max_drawdown_pct"] = float(line.split("Max Drawdown:")[1].strip().replace("%",""))
            except Exception:
                pass
        ...
        if "Win Rate:" in line:
            try:
                metrics["win_rate_pct"] = float(line.split("Win Rate:")[1].strip().replace("%",""))
            except Exception:
                pass
...
def main() -> None:
    ...
    out_file = OUT_DIR / f"multi_pair_1h_{BARS}bars_{timestamp}.json"
    ...
    print(f"\n{'='*80}")
    print(f"📊 Multi-Pair 1h (last {BARS} bars) Summary — {timestamp}")
    print(f"{'='*80}")
    print(f"{'Symbol':<12} {'Final Eq':>12} {'Return%':>10} {'MaxDD%':>10} {'Sharpe':>8} {'Trades':>8} {'Win%':>8}")
    print("-"*80)
    for r in results:
        if "error" in r:
    ...
    baseline_files = sorted(OUT_DIR.glob("multi_pair_1h_*bars_*.json"))
