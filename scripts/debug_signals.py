...
for ts in [
    ...
    orch = Orchestrator()
    r = orch.analyze(
        symbol="BTC/USDT", timeframe="1h", current_position_pct=0.0, df=df.head(idx + 1)
    )
    d = r.final_decision
    ...
    print(
        f"{ts} | MA20={ma20:.0f} MA50={ma50:.0f} RSI={rsi:.1f} | final={d.signal} conf={d.confidence:.2f} risk={d.risk_level}"
    )
    print(f"   {sigs}")
