...
def main():
    ...
    print(
        f"[CŨ]  thiếu trade_count 1 bên → score={report_old.score:.3f}, gate={report_old.pass_gate}"
    )
    ...
    print(
        f"[MỚI] thiếu trade_count 1 bên → score={report_new.score:.3f}, gate={report_new.pass_gate}"
    )
    ...
    mem.add(a1)
    print(
        f"[CŨ]  in-memory: {len(mem.all_for('ma'))} artifacts, không persist, không chống sửa"
    )
    ...
    print(
        f"[CŨ]  lifecycle không cần evidence: EXPLORATORY → CANARY_LIVE OK ({len(lc_old.events)} transitions)"
    )
    ...
    try:
        ...
        policy.validate_evidence(
            "art-new", PromotionState.EXPLORATORY, PromotionState.REVIEWED, []
        )
    ...
    policy.validate_evidence(
        ...
        [manual_review_evidence("reviewed")],
    )
    ...
    policy.validate_evidence(
        ...
        [reality_gap_evidence(rg), drift_check_evidence(health.value)],
    )
    lc_new.transition(PromotionState.CANARY_ELIGIBLE)
    print(
        f"[MỚI] đủ evidence → {lc_new.state.value} ✅, evidence lưu: {len(policy.get_evidence('art-new'))} items"
    )
    ...
    print(
        f"[CŨ]  passive_fill_prob={cfg_old.passive_fill_prob}, impact_coeff={cfg_old.impact_coeff} (mặc định cứng)"
    )
    ...
    print(
        f"[MỚI] calibrated từ 30 fills: passive_fill_prob={cfg_new.passive_fill_prob:.3f}"
    )
    print(
        f"     impact_coeff={cfg_new.impact_coeff:.3f}, adverse={cfg_new.adverse_selection_bps:.2f}bps"
    )
    ...
    print(
        f"     can_increase_exposure={signal.can_increase_exposure} (boolean, không chi tiết)"
    )
    ...
    for appetite in ("aggressive", "moderate", "conservative"):
        ...
        print(
            f"       {appetite:12s} → allowed: {allowed}, rec: {p.recommended_action(decision).value}"
        )

    # Backward compat adapter
    adapter = ThresholdDecisionPolicy("moderate")
    print(
        f"[MỚI] adapter legacy: can_increase={adapter.can_increase(signal)} (vẫn dùng được như cũ)"
    )
