...
def _resolve_run() -> tuple[str, Path]:
    ...
    if (
        latest.exists()
        and (latest / "walk_forward_results.json").exists()
        and (latest / "data_quality" / "audit.json").exists()
    ):
...
def multi_timeframe_context() -> list[dict]:
    ...
    for sym in SYMBOLS:
        ...
        df_4h_closed = (
            df_4h.filter(pl.col("is_closed")) if "is_closed" in df_4h.columns else df_4h
        )
        df_1d_closed = (
            df_1d.filter(pl.col("is_closed")) if "is_closed" in df_1d.columns else df_1d
        )
        ...
        for tf in ["4h", "1d"]:
            ...
            for f_delta in [-0.2, 0, 0.2]:
                for s_delta in [-0.2, 0, 0.2]:
                    ...
                    slow = max(
                        fast + 1, int(base_params["slow_period"] * (1 + s_delta))
                    )
                    try:
...
def portfolio_construction() -> list[dict]:
    ...
    candidates = [
        r
        for r in wfo
        if r.get("oos_sharpe", 0) > 0 and r.get("status") == "PAPER_ELIGIBLE"
    ]
    if not candidates:
        candidates = sorted(
            wfo, key=lambda x: x.get("net_return", -9999), reverse=True
        )[:5]
...
def final_holdout() -> list[dict]:
    ...
    for r in wfo:
        ...
        sym_folds = [
            f
            for f in folds
            if f["symbol"] == r["symbol"] and f["timeframe"] == r["timeframe"]
        ]
        if not sym_folds:
            continue
        last = sym_folds[-1]
...
def update_final_report(
    ...
    for a in audit:
        lines.append(
            f"| {a['symbol']} | {a['timeframe']} | {a.get('quality', '')} | {a.get('bars', 0)} | {a.get('gaps', 0)} |"
        )
    lines.append("")
    ...
    for sym in SYMBOLS:
        for tf in TIMEFRAMES:
            ...
            lines.append(
                f"| {sym} | {tf} | {best.get('strategy', '')} | {best.get('oos_sharpe', 0):.2f} | {best.get('net_return', 0):.2f}% | {best.get('max_dd', 0):.2f}% | {best.get('dsr', 0):.2f} | {best.get('status', '')} |"
            )
    lines.append("")
    ...
    for sym in SYMBOLS:
        ...
        if best:
            lines.append(
                f"| {sym} | {best.get('timeframe', '')} | {best.get('strategy', '')} | {best.get('oos_sharpe', 0):.2f} | {best.get('net_return', 0):.2f}% | {best.get('max_dd', 0):.2f}% | {best.get('dsr', 0):.2f} | {best.get('cost_2x_sharpe', 0):.2f} | {best.get('cost_3x_sharpe', 0):.2f} | {best.get('status', '')} |"
            )
    lines.append("")
    ...
    lines.append(
        "_Baseline 1h-only results collected. Full MTF context pipeline requires feature-store integration._\n"
    )
    ...
    for r in param_stability[:20]:
        lines.append(
            f"| {r['symbol']} | {r['timeframe']} | {r['strategy']} | {r['variations']} | {r.get('min_sharpe', 0):.2f} | {r.get('max_sharpe', 0):.2f} | {r.get('mean_sharpe', 0):.2f} | {r.get('std_sharpe', 0):.2f} | {r.get('stability_score', 0):.2f} |"
        )
    lines.append("")
    ...
    for r in regime[:30]:
        lines.append(
            f"| {r['symbol']} | {r['timeframe']} | {r['regime']} | {r['mom20']:.4f} | {r['mom60']:.4f} | {r['vol20']:.4f} |"
        )
    lines.append("")
    ...
    for r in portfolio:
        lines.append(
            f"| {r['symbol']} | {r['timeframe']} | {r['strategy']} | {r['weight']:.2f} | {r.get('oos_sharpe', 0):.2f} | {r.get('net_return', 0):.2f}% | {r.get('max_dd', 0):.2f}% |"
        )
    lines.append("")
    ...
    for r in stress[:20]:
        lines.append(
            f"| {r['symbol']} | {r['timeframe']} | {r['strategy']} | {r['base_sharpe']:.2f} | {r['fees_x2']:.2f} | {r['fees_x3']:.2f} | {r['slippage_x2']:.2f} | {r['delayed_entry']:.2f} |"
        )
    lines.append("")
    ...
    for r in holdout[:20]:
        lines.append(
            f"| {r['symbol']} | {r['timeframe']} | {r['strategy']} | {r.get('holdout_sharpe', 0):.2f} | {r.get('holdout_return', 0):.2f}% | {r.get('holdout_max_dd', 0):.2f}% | {r.get('holdout_trades', 0)} | {r['status']} |"
        )
    lines.append("")
