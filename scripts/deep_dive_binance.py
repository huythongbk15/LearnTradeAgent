...
def hhv(df: pl.DataFrame, col: str, n: int) -> pl.Series:
    return df.select(pl.col(col).rolling_max(window_size=n))[col].alias(f"hhv_{col}_{n}")
...
def analyze(symbol: str) -> dict:
    ...
    return {
        "symbol": symbol,
        "price": close[i],
        "ma_fast": ma_fast, "ma_slow": ma_slow, "bull": bull,
        "adx": adx, "regime": regime,
        "rsi": rsi, "atr_pct": atr_pct, "vol_ratio": vol_ratio,
        "dist_hi90": dist_hi, "dist_lo90": dist_lo,
        "state": "LONG" if in_pos else "FLAT",
        "n_buy24h": n_buy24h, "n_sell24h": n_sell24h,
        "ret_90d": ret_strat, "ret_bh": ret_bh, "max_dd": dd, "n_trades": n_trades,
    }
...
def main():
    ...
    print("=" * 118)
    print("🔬 DEEP-DIVE — Binance PUBLIC 1h | Enhanced MA(20,80,ADX40) + RSI/ATR/Volume/Regime")
    print("=" * 118)
    hdr = (f"{'SYMBOL':<11}{'PRICE':>10}{'MA20/MA80':>9}{'BULL':>5}{'ADX':>6}{'REGIME':>11}"
    ...
    for s in syms:
        try:
            r = analyze(s)
            rows.append(r)
            print(f"{r['symbol']:<11}{r['price']:>10,.1f}{r['ma_fast']:>9,.0f}/{r['ma_slow']:>5,.0f}"
    ...
    for r in rows:
        verdict = "🔥 +EDGE" if r["ret_90d"] > r["ret_bh"] + 5 and r["max_dd"] > -25 else ("⚠️ SLOW/NOISE" if r["max_dd"] < -40 else "➡️ BÌNH THƯỜNG")
        print(f"  {r['symbol']:<11} strat {r['ret_90d']:>7,.1f}% | buy&hold {r['ret_bh']:>7,.1f}% | maxDD {r['max_dd']:>6,.1f}% | trades {r['n_trades']:>4}  {verdict}")
