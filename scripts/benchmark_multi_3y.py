...
PAIRS=[('BTC/USDT','1h'),('ETH/USDT','1h'),('XRP/USDT','1h'),('BTC/USDT','4h'),('ETH/USDT','4h'),('XRP/USDT','4h')]
out={}
for sym,tf in PAIRS:
    print(f'=== {sym} {tf} ===')
    t0=time.time()
    try:
        res=walk_forward_optimize(symbol=sym, timeframe=tf, train_months=12, test_months=3, step_months=3, top_n=5, fast=True)
