...
def main() -> None:
    ...
    print(
        f"  Return: {old.total_return_pct:+.2f}% | Sharpe: {old.sharpe_ratio:.2f} | Trades: {old.total_trades}"
    )
    ...
    print(
        f"  Return: {res.metrics.total_return_pct:+.2f}% | Sharpe: {res.metrics.sharpe:.2f}"
    )
    ...
    ident = (
        abs((att.theoretical_alpha_pnl - att.execution_cost) - att.realized_pnl) < 1e-6
    )
    ...
    ref_metrics = {
        ...
        "trade_count": float(old.total_trades),
        "turnover": 0.0,
        "avg_latency_ms": 0.0,
        "spread_cost_quote": 0.0,
        "fees_quote": 0.0,
        "sharpe": float(old.sharpe_ratio),
        "total_return_pct": float(old.total_return_pct),
    ...
    print(
        f"  Artifact immutable: id={art.artifact_id[:12]}... param_hash={art.parameter_hash[:12]}..."
    )
    try:
    ...
    print(
        f"  Rename strategy (ma_v1->ma_v2): total_trials={reg.total_trials()} (KHÔNG reset)"
    )

    hi = UncertaintySignal(
    ...
    print(
        f"  Uncertainty HIGH -> can_increase_exposure={hi.can_increase_exposure} (phải False)"
    )
    print(
        f"  Uncertainty LOW  -> can_increase_exposure={lo.can_increase_exposure} (phải True)"
    )
    ab = should_abstain(
        symbol="BTC/USDT",
        strategy="ma_crossover",
        reason=AbstentionReason.OOD_INPUT,
        uncertainty=hi,
    )
