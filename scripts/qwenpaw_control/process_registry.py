...
def _init_db():
    with sqlite3.connect(DB_PATH) as con:
        ...
        con.execute(
            "CREATE INDEX IF NOT EXISTS idx_status ON qwenpaw_processes(status)"
        )
        con.execute(
            "CREATE INDEX IF NOT EXISTS idx_heartbeat ON qwenpaw_processes(heartbeat_at)"
        )
...
def register(pid: int, cmd: List[str], meta: Dict = None) -> ProcessRecord:
    ...
    with _db() as con:
        con.execute(
            """INSERT OR REPLACE INTO qwenpaw_processes 
...
def heartbeat(pid: int):
    ...
    with _db() as con:
        con.execute(
            "UPDATE qwenpaw_processes SET heartbeat_at=? WHERE pid=?",
            (time.time(), pid),
        )
        con.commit()
...
def complete(pid: int, status: str, result_file: str = ""):
    ...
    with _db() as con:
        con.execute(
            "UPDATE qwenpaw_processes SET status=?, result_file=? WHERE pid=?",
...
def get(pid: int) -> Optional[ProcessRecord]:
    with _db() as con:
        row = con.execute(
            "SELECT * FROM qwenpaw_processes WHERE pid=?", (pid,)
        ).fetchone()
        if row:
...
def list_active() -> List[ProcessRecord]:
    with _db() as con:
        rows = con.execute(
            "SELECT * FROM qwenpaw_processes WHERE status='running' ORDER BY started_at"
        ).fetchall()
...
def list_recent(limit: int = 20) -> List[ProcessRecord]:
    with _db() as con:
        rows = con.execute(
            "SELECT * FROM qwenpaw_processes ORDER BY started_at DESC LIMIT ?", (limit,)
        ).fetchall()
        return [ProcessRecord(**dict(r)) for r in rows]
...
def cleanup_stale(max_age_sec: int = 14400, kill: bool = True) -> int:
    ...
    with _db() as con:
        rows = con.execute(
            "SELECT pid, cmd FROM qwenpaw_processes WHERE status='running'"
        ).fetchall()
        for row in rows:
            pid, cmd = row
            hb_row = con.execute(
                "SELECT heartbeat_at FROM qwenpaw_processes WHERE pid=?", (pid,)
            ).fetchone()
...
if __name__ == "__main__":
    ...
    if cmd == "list":
        for r in list_active():
            ...
            print(
                f"PID={r.pid:6d} | age={age:5d}s | hb_age={hb_age:4d}s | type={meta.get('type', '?')} | {r.cmd[:100]}"
            )
    elif cmd == "recent":
        for r in list_recent(20):
            age = int(time.time() - r.started_at)
            print(
                f"PID={r.pid:6d} | {time.ctime(r.started_at)} | {r.status:12s} | {r.cmd[:80]}"
            )
    elif cmd == "cleanup":
        n = cleanup_stale()
