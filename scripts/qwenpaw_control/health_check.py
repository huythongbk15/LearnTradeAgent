...
def health_check() -> dict:
    ...
    active = list_active()
    stale_count = sum(
        1 for p in active if time.time() - p.heartbeat_at > 300
    )  # 5min no heartbeat
    checks["process_registry"] = {
        ...
        "details": [
            {
                "pid": p.pid,
                "age_sec": int(time.time() - p.started_at),
                "hb_age_sec": int(time.time() - p.heartbeat_at),
                "cmd": p.cmd[:60],
            }
            for p in active[:10]
        ],
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline", "create_time"]):
        try:
            ...
            ).lower() and "app" in " ".join(proc.info["cmdline"] or []):
                main_proc = proc.info
                break
        except Exception:
            pass
    ...
    checks["qwenpaw_main"] = {
        ...
        "uptime_sec": int(time.time() - main_proc["create_time"])
        if main_proc
        else None,
    }
    if not main_proc:
    ...
    return {"overall": overall, "timestamp": time.time(), "checks": checks}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--format", choices=["json", "nagios", "prometheus"], default="json"
    )
