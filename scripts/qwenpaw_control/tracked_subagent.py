...
class TrackedSubagent:
    ...
    def spawn(self, task: str, timeout: int = 600, 
              allowed_tools: list = None, skills: list = None,
              background: bool = False, fork: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Update registry
            complete(registry_pid, "submitted", result_file=f"subagent_{self.task_id}.json")
            
            # Save submission result
            self._save_result({"submitted": True, "task_id": self.task_id, "meta": meta})
            
            return {"status": "submitted", "task_id": self.task_id, "registry_pid": registry_pid}
            
        except Exception as e:
            complete(registry_pid, "failed", error=str(e))
            return {"status": "failed", "error": str(e), "registry_pid": registry_pid}
    
    def wait(self, task_id: str = None, poll_interval: int = 10, max_wait: int = 3600) -> Dict[str, Any]:
        ...
        while time.time() - start < max_wait:
            ...
            try:
                ...
                if result.get("status") in ("completed", "failed", "error"):
                    ...
                    if self.registry_pid:
                        complete(self.registry_pid, result.get("status", "completed"), 
    ...
    def _save_result(self, result: Dict):
        ...
        fname = out_dir / f"subagent_{self.task_id or 'unknown'}_{int(time.time())}.json"
...
if __name__ == "__main__":
    ...
    tracker = TrackedSubagent()
    result = tracker.spawn(args.task, timeout=args.timeout, background=args.background,
