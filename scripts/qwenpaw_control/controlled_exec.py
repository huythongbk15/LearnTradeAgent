...
class ControlledExec:
    def __init__(
        self, workspace: Path, timeout_sec: int = 3600, heartbeat_sec: int = 30
    ):
    ...
    def run(
        ...
        stdout_thread = threading.Thread(
            target=self._read_stream, args=(self._proc.stdout, "stdout")
        )
        stderr_thread = threading.Thread(
            target=self._read_stream, args=(self._proc.stderr, "stderr")
        )
        stdout_thread.start()
        stderr_thread.start()
        ...
        try:
            while True:
                ...
                if time.time() - self._last_heartbeat > self.heartbeat_sec:
                    ...
                    print(
                        f"[QWENPAW HEARTBEAT] PID={self._proc.pid} elapsed={elapsed}s cmd={' '.join(cmd[:3])}...",
                        flush=True,
                    )
            ...
            return ExecResult(
                ...
                rc,
                time.time() - self._start_time,
                self._output_lines[-100:],
                self._error_lines[-100:],
                result_path,
            )
    ...
    def _terminate(self, reason: str):
        if self._proc and self._proc.poll() is None:
            print(
                f"[QWENPAW] Terminating PID={self._proc.pid} reason={reason}",
                flush=True,
            )
