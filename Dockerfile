...
COPY pyproject.toml requirements-web.txt requirements.lock ./
# Copy source code needed for the locked install (deps + project itself)
COPY src/ ./src/

# Install production dependencies from the *locked* hashed requirements
# (generated by `uv export` — exact transitive versions, deterministic
# build). The project package itself is installed from source with --no-deps
