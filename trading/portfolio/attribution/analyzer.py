...
class AttributionAnalyzer:
    ...
    def factor_attribution(
        ...
        # Align
        common = portfolio_returns.index.intersection(factor_returns.index).intersection(factor_loadings.index)
...
class StrategyAttribution:
    ...
    def attribute(
        ...
        if self.strategy_weights:
            ...
            weights_df = pd.DataFrame(1.0 / len(strat_ret.columns), index=df.index, columns=strat_ret.columns)
        ...
        for col in strat_ret.columns:
            total_contrib[col] = Decimal(str(
                (weights_df[col] * strat_ret[col]).sum()
            ))
...
class AssetClassAttribution:
    ...
    def attribute(self, portfolio_returns: pd.Series) -> dict[str, Decimal]:
        ...
        if self.asset_weights:
            ...
            weights = pd.DataFrame(1.0 / len(df.columns), index=df.index, columns=df.columns)
