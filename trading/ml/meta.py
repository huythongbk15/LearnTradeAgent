...
class MAML(MetaLearner):
    ...
    def _inner_update(self, task: Task, params: dict, X: np.ndarray, y: np.ndarray) -> dict:
    ...
    def meta_train(self, tasks: list[Task], steps: int) -> dict:
        ...
        for step in range(steps):
            ...
            batch_tasks = random.sample(tasks, min(self.config.meta_batch_size, len(tasks)))
            ...
            for key in self.meta_params:
                self.meta_params[key] -= self.config.meta_lr * meta_grads[key] / len(batch_tasks)
    ...
    def meta_train(self, tasks: list[Task], steps: int) -> dict:
        ...
        for step in range(steps):
            ...
            for task in batch_tasks:
                ...
                for key in self.meta_params:
                    self.meta_params[key] += self.config.reptile_meta_lr * (params[key] - self.meta_params[key])
...
class MetaSGD(MetaLearner):
    ...
    def __init__(self, config: MetaLearningConfig, initial_params: dict, initial_lr: Optional[dict] = None):
        super().__init__(config)
        self.meta_params = initial_params.copy()
        self.meta_lr_params = initial_lr or {key: config.inner_lr for key in initial_params}
        ...
        for step in range(steps):
            ...
            for task in batch_tasks:
                ...
                for key in self.meta_params:
                    ...
                    meta_lr_grads[key] += -query_grad.get(key, 0) * support_grad.get(key, 0)
            
            # Update meta-parameters and learning rates
            for key in self.meta_params:
                self.meta_params[key] -= self.config.meta_lr * meta_param_grads[key] / len(batch_tasks)
                self.meta_lr_params[key] -= self.config.meta_lr * meta_lr_grads[key] / len(batch_tasks)
...
class ANIL(MetaLearner):
    ...
    def __init__(self, config: MetaLearningConfig, initial_params: dict, head_keys: list[str]):
        super().__init__(config)
        self.meta_params = initial_params.copy()
...
class MetaStrategyAdapter:
    ...
    def __init__(self, algorithm: str = "maml", config: Optional[MetaLearningConfig] = None):
    ...
    def train(self, market_data: dict[str, np.ndarray], steps: int = 100) -> dict:
        ...
        if self.algorithm == "maml":
            ...
            self.learner = ANIL(self.config, initial_params, head_keys=["ema_fast", "ema_slow"])
        else:
