...
class StrategyRegistry:
    ...
    def register(
        ...
        if metadata.backtest_hash:
            ...
            expected_hash = self._compute_hash(source_code + json.dumps(metadata.params_schema, sort_keys=True))
        ...
        logger.info(f"Registered strategy {name} v{metadata.version} (hash: {source_hash[:8]})")
        return version
...
class StrategyLoader:
    ...
    def load(self, name: str, version: Optional[str] = None) -> type[Strategy]:
        ...
        import trading.strategies.plugins as plugins_module
    ...
    def load_with_params(self, name: str, params: dict, version: Optional[str] = None) -> Strategy:
